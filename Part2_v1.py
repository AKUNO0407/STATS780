...
comp = ['Order Discrepancy', 'Cancellation Rate', 'Missed Orders Rate', 'Churned','Total_Order_Value_norm',
        'MRR Score', 'Loyalty_norm','Normalized Retention Score', 'Delivery Partner Score']
...
def customer_accounts_view(data1):
    ...
    finance_75p =  np.percentile(data1[['Delivery Partner Score', 'MRR Score','Total_Order_Value_norm']].dot([10,30,25]), 75)
    ...
    df_finance_75p =  filtered_data_csa[filtered_data_csa[['Delivery Partner Score', 'MRR Score','Total_Order_Value_norm']].dot([10,30,25]) >= finance_75p]
    df_churn = filtered_data_csa[filtered_data_csa['Churned'] == 1]
    df_good_cus = df_finance_75p[(df_finance_75p['Loyalty'] >= loyalty_75) & (df_finance_75p['Health_Score'] >= 70)]
   ...
   #     st.dataframe(filtered_data_csa[['Parent Restaurant name','Unique Location ID', 'Health_Score']].groupby(['Parent Restaurant name','Unique Location ID']).mean().style.apply(color_coding, axis=1))
    with cl2:
        ...
        st.markdown("'High Value Customers': Customers above 75 percentile of Loyalty Score and Financial Performance with Health Score >= 70")
